...
def Paolo_autofit_global_50(group: QuadrantGroup, use_corrected=False, save_path=None): 
    ...
    csv_file = '/cosma/home/durham/dc-barr6/richard_scripts/sunspot_daily_2025.csv'
    ...
    def sunspot_rho_q_timelag(dates_since_launch, 
                              param_vals = [0.08981, 5.5738e-04, -3.6802e-06, 0.83093], 
                              t_lag=430
                             ):
        # Calculate number of traps in HST/ACS, based on daily sunspot activity from https://www.sidc.be/SILSO/datafiles
        # 
        # Parameters are
        # 0: Density of traps at launch [pixel^-1]
        # 1, 2, 3: Growth rate of traps each day (t, where launch is t=0) is d rho/dt = p[1] + p[2] * n_sunspot(t-t_lag)^p[3]
        # 4: (optionally) time lag from damage-inducing Solar radiation to visible sunspots [days]
        #
        # Uncertainties on default param_values are [4.50934640e-03, 9.82183946e-07, 3.33677185e-08, 1.97824640e-03]
        dates_since_launch_list=[]
        dates_since_launch_list.append(dates_since_launch)
        param_vals = list(param_vals)+[t_lag]
        sunspot_days_lagged = sunspot_days_since_launch_fit_array + param_vals[4]
        rho_q_vals = []
        for date in dates_since_launch_list:
            t_frac = param_vals[4] - int(param_vals[4]) # Extract any non-integer part of the time lag
            if t_frac > 1e-6:
                # Interpolate between two integer time lags
                param_vals_copy = param_vals.copy()
                param_vals_copy[4] = int(param_vals_copy[4]) 
                rho_q_1 = sunspot_rho_q_timelag( [date], param_vals_copy )
                param_vals_copy[4] += 1
                rho_q_2 = sunspot_rho_q_timelag( [date], param_vals_copy )
                rho_q = ( (1-t_frac) * rho_q_1 ) + ( t_frac * rho_q_2 )
                #print(rho_q_1,rho_q_2,rho_q, t_frac, param_vals[4])
            else:
                # Generate times between ACS launch minus lag and current date minus lag
                t_vals = sunspot_days_since_launch_fit_array[ ( sunspot_days_lagged >= 0 ) & ( sunspot_days_lagged <= date ) ]
                # Look up number of sunspots on all those days
                selected_spots = sunspot_interp(t_vals)
                # Integrate growth rate from launch to current date
                rho_q = param_vals[0] + np.sum(param_vals[1] + param_vals[2] * (selected_spots) ** param_vals[3] )
            rho_q_vals.append( rho_q )
        return float(rho_q_vals[0])
    
    # Convert MJD to days since launch for notch time evolution
    global days_var
    JD_var=float(MJD_var)+2400000.5
    days_var=JD_var-2452334.5 #days since launch
    
    rho_q = sunspot_rho_q_timelag(days_var, 
                              param_vals = [0.08981, 5.5738e-04, -3.6802e-06, 0.83093], 
                              t_lag=430
                             )
...
#     elif days_var < 3961.92584_notch511_cosine00728617:
#         rho_q=rho_q-0.037586665739412245+2.74840460582629e-05*(days_var-1586.5) # m2=vals2 c1=vals4
#     elif days_var > 3961.92584_notch511_cosine00728617:
#         rho_q=rho_q-0.037586665739412245+2.74840460582629e-05*(3961.92584_notch511_cosine00728617-1586.5)-9.500901967932416e-06*(days_var-3961.92584_notch511_cosine00728617) #m3=vals3 t2=vals5    
# =============================================================================
...
# =============================================================================
    ...
    writefilename=f"{dataset_date}_2024_july_opt2_beta584_notch511_sunspot_{const_fix}" 
    ...
    csv_list=[x for x in csvs_string if f"{dataset_date}_2024_july_opt2_beta584_notch511_sunspot_{const_fix}" in x]
    print(csv_list)
    csv_name=str(os.path.basename(csv_list[0]))
    print(csv_name)
    target2=path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts", f"2024_july_opt2_beta584_notch511_sunspot_{const_fix}", "csv_files",
                     str(csv_name))
...
cosma_dataset_path = path.join(cosma_path, "hst", "cte_all", dataset_date)
cosma_output_path = path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts", f"2024_july_opt2_beta584_notch511_sunspot_{const_fix}")
...
base_dir = os.path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts", f"2024_july_opt2_beta584_notch511_sunspot_{const_fix}")
os.makedirs(base_dir, exist_ok=True)

sub_dir_1 = os.path.join(base_dir, f"{dataset_date}_2024_july_opt2_beta584_notch511_sunspot_{const_fix}")
...
Paolo_autofit_global_50(
    group,
    save_path=Path(path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts", f"2024_july_opt2_beta584_notch511_sunspot_{const_fix}",
                     f"{dataset_date}_2024_july_opt2_beta584_notch511_sunspot_{const_fix}"))/f"{dataset_date}_2024_july_opt2_beta584_notch511_sunspot_{const_fix}.png"
)
...
for file in files_bia:
    ...
    output_path = path.join(path.sep, "cosma5", "data", "durham", "dc-barr6","richard_scripts", f"2024_july_opt2_beta584_notch511_sunspot_{const_fix}", 
                            f"{dataset_date}_2024_july_opt2_beta584_notch511_sunspot_{const_fix}", filename)
...
for file in files:
    ...
    image_A, image_B, image_C, image_D = [
        ImageACS.from_fits(
            ...
            bias_file_path=path.join(path.sep, "cosma5", "data", "durham", "dc-barr6","richard_scripts", f"2024_july_opt2_beta584_notch511_sunspot_{const_fix}", 
                                    f"{dataset_date}_2024_july_opt2_beta584_notch511_sunspot_{const_fix}")
        ).native
        for quadrant in ["A", "B", "C", "D"]
    ]
...
# =============================================================================
    ...
    #  Print results to csv file 
    writefilename=f"{dataset_date}_2024_july_opt2_beta584_notch511_sunspot_{const_fix}_corrected"
    with open(writefilename+'.csv', 'w', newline='') as file:
        writer = csv.writer(file)
    ...
    for stuff in csvs_all:
        csvs_string.append(str(stuff))
    csv_list=[x for x in csvs_string if f"{dataset_date}_2024_july_opt2_beta584_notch511_sunspot_{const_fix}_corrected" in x]
    print(csv_list)
    csv_name=str(os.path.basename(csv_list[0]))
    print(csv_name)
    target3=path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts",f"2024_july_opt2_beta584_notch511_sunspot_{const_fix}",
                     "csv_files", str(csv_name))
    shutil.copyfile(csv_list[0],target3)

# Import data to be fitted
cosma_dataset_path = path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts",f"2024_july_opt2_beta584_notch511_sunspot_{const_fix}",
                               f"{dataset_date}_2024_july_opt2_beta584_notch511_sunspot_{const_fix}")
...
Paolo_autofit_global_50_after(
    group,
    save_path=Path(path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts", f"2024_july_opt2_beta584_notch511_sunspot_{const_fix}",
                     f"{dataset_date}_2024_july_opt2_beta584_notch511_sunspot_{const_fix}"))/f"{dataset_date}_2024_july_opt2_beta584_notch511_sunspot_{const_fix}_corrected.png"
)
