...
sunspot_fit=[]
csv_file = r'C:\Users\chipe\Documents\Durham University\warm_pixels\scripts\sunspot_monthly.csv'
...
with open(csv_file, mode='r') as file:
    ...
    for row in csv_reader:
        ...
        if float(row[0]) > 2001:
            if float(row[0]) not in [2004, 2008, 2012, 2016, 2020, 2024]: # Check if leap year
                leap_year = False
            elif float(row[0]) in [2004, 2008, 2012, 2016, 2020, 2024]: 
                leap_year = True
            if float(row[1]) in [1, 3, 5, 7, 8, 10, 12]:
                for i in range(31): # these months have 31 days
                    sun_years.append(row[0])
                    sun_months.append(row[1])
                    sun_days.append(i+1)
                if float(row[3]) > 0: # check we have positive number of sunspots
                    for i in range(31):
                        sunspots.append(row[3])
                else: 
                    for i in range(31):
                        sunspots.append(0)
            elif float(row[1]) in [4, 6, 9, 11]:
                for i in range(30): # these months have 30 days
                    sun_years.append(row[0])
                    sun_months.append(row[1])
                    sun_days.append(i+1)
                if float(row[3]) > 0:
                ...
                for i in range(feb_days): # adjust to number of days in Feb
                    sun_years.append(row[0])
                    sun_months.append(row[1])
                    sun_days.append(i+1)
                if float(row[3]) > 0:
...
sunspot_fit_array = np.array(sunspot_fit, dtype=float)
sunspot_days_since_launch_fit_array = np.array(sunspot_days_since_launch_fit)

print("Sunspot data extracted from csv file!")
        
        
def sunspot_rho_q(dates_since_launch, param_vals):
    rho_q_vals = []
    for date in dates_since_launch:
        mask = sunspot_days_since_launch_fit_array <= date
        selected_spots = sunspot_fit_array[mask]
