...
def Paolo_autofit_global_50(group: QuadrantGroup, use_corrected=False, save_path=None): 
    ...
    def sunspot_rho_q_cosine(dates_since_launch, 
                            #param_vals = [-0.0104, 0.0004669, 0.04391, -282.3], #v1, May 2025
                            param_vals = [-1.91501315e-02,  4.67419398e-04,  5.51402892e-02, -2.11185688e+02],
                            cycle_length = 11.0
                           ):    
       # Linear growth of traps, plus perturbations an 11 year cycle
       # 
       # Parameter values should be
       # 0, 1: y=mx+t
       # 2: amplitude of sinusoidal variations
       # 3: time lag of sinusoidal variations [years], relative to launch date (around the peak of a Solar cycle)
       # 4: (optionaly) the length of a Solar cycle [years]. If undefined, this uses 11 years.
       #
       # Uncertainties on default param_values are [0.0001612, 3.587e-08, 0.0001342, 0.0002173]
       param_vals = list(param_vals)+[cycle_length]
       rho_q_vals = (
           param_vals[0] + 
           param_vals[1] * np.array(dates_since_launch) + 
           param_vals[2] * np.cos( 2 * np.pi * (np.array(dates_since_launch) - param_vals[3]) / ( param_vals[4] * 365.25) )
       )
       return rho_q_vals
    ...
    rho_q = sunspot_rho_q_cosine(days_var, param_vals = [-0.0104, 0.0004669, 0.04391, -282.3], 
    cycle_length = 11.0)
...
# =============================================================================
    ...
    writefilename=f"{dataset_date}_2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}" 
    ...
    csv_list=[x for x in csvs_string if f"{dataset_date}_2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}" in x]
    print(csv_list)
    csv_name=str(os.path.basename(csv_list[0]))
    print(csv_name)
    target2=path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts", f"2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}", "csv_files",
                     str(csv_name))
...
cosma_output_path = path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts", f"2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}")
...
base_dir = os.path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts", f"2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}")
os.makedirs(base_dir, exist_ok=True)

sub_dir_1 = os.path.join(base_dir, f"{dataset_date}_2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}")
...
Paolo_autofit_global_50(
    group,
    save_path=Path(path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts", f"2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}",
                     f"{dataset_date}_2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}"))/f"{dataset_date}_2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}.png"
)
...
for file in files_bia:
    ...
    output_path = path.join(path.sep, "cosma5", "data", "durham", "dc-barr6","richard_scripts", f"2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}", 
                            f"{dataset_date}_2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}", filename)
...
for file in files:
    ...
    image_A, image_B, image_C, image_D = [
        ImageACS.from_fits(
            ...
            bias_file_path=path.join(path.sep, "cosma5", "data", "durham", "dc-barr6","richard_scripts", f"2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}", 
                                    f"{dataset_date}_2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}")
        ).native
        for quadrant in ["A", "B", "C", "D"]
    ]
...
# =============================================================================
    ...
    #  Print results to csv file 
    writefilename=f"{dataset_date}_2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}_corrected"
    with open(writefilename+'.csv', 'w', newline='') as file:
        writer = csv.writer(file)
    ...
    for stuff in csvs_all:
        csvs_string.append(str(stuff))
    csv_list=[x for x in csvs_string if f"{dataset_date}_2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}_corrected" in x]
    print(csv_list)
    csv_name=str(os.path.basename(csv_list[0]))
    print(csv_name)
    target3=path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts",f"2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}",
                     "csv_files", str(csv_name))
    shutil.copyfile(csv_list[0],target3)

# Import data to be fitted
cosma_dataset_path = path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts",f"2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}",
                               f"{dataset_date}_2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}")
...
Paolo_autofit_global_50_after(
    group,
    save_path=Path(path.join(path.sep, "cosma5", "data", "durham", "dc-barr6", "richard_scripts", f"2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}",
                     f"{dataset_date}_2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}"))/f"{dataset_date}_2024_july_opt2_beta584_notch511_cosine_v2_{const_fix}_corrected.png"
)
