...
# 1 BIG FIT FOR PRE CORRECTION RHO Q 
# =============================================================================
# t1_rho_days_list=[]
# t2_rho_days_list=[]
# t3_rho_days_list=[]
...
for i in range(len(ccdgains)):
        ...
        rho_fit_vals_list.append(rho_q_pres[i])
        if rho_q_pre_lower[i] > rho_q_pre_upper[i]:
            rho_fit_errors_list.append(rho_q_pre_lower[i])
        else:
            rho_fit_errors_list.append(rho_q_pre_upper[i])
...
def rho_fit(x, param_vals):
    calc_vals=[]
    for days in x:
        if days < param_vals[5]:
            calc_vals.append(param_vals[0]*days+param_vals[1]) #m1 = vals0, c0 = vals1
        elif days > param_vals[5] and days < param_vals[6]:
            calc_vals.append(param_vals[2]*(days-param_vals[5])+param_vals[0]*param_vals[5]+param_vals[1]) # m2=vals2 t1=vals5
        elif days > param_vals[6] and days < param_vals[7]:
            calc_vals.append(param_vals[3]*(days-param_vals[6])+param_vals[2]*(param_vals[6]-param_vals[5])+param_vals[0]*param_vals[5]+param_vals[1]) #m3=vals3 t2=vals6
        elif days > param_vals[7]:
            calc_vals.append(param_vals[4]*(days-param_vals[7])+param_vals[3]*(param_vals[7]-param_vals[6])+param_vals[2]*(param_vals[6]-param_vals[5])+param_vals[0]*param_vals[5]+param_vals[1]) #m4=vals4 t3=vals7
    return (calc_vals)

print('RHO_Q FIT RESULTS')
initial_values=np.array([0,0,0,0,0,1366,4166,5166])
deg_freedom = len(rho_fit_days_list) - initial_values.size
print('DoF = {}'.format(deg_freedom))
bnds = ((0,10) ,(-10, 10),(0,10),(0,10),(0,10),(0,8000),(0,8000),(0,8000))
fit = scipy.optimize.minimize(chi_squared, initial_values, method='L-BFGS-B',args=(rho_fit, rho_fit_days, rho_fit_vals, 
                                                                  rho_fit_errors), bounds=bnds)
print(fit.success) 
print(fit.message) 
sol0 = fit.x[0]
sol1 = fit.x[1]
...
sol6 = fit.x[6]
sol7 = fit.x[7]
fit_line_rho = rho_fit(rho_fit_days, [sol0,sol1,sol2,sol3,sol4,sol5,sol6,sol7])
...
P_value = scipy.stats.chi2.sf(chisq_min, deg_freedom)
print('P(chi^2_min, DoF) = {}'.format(P_value))
print('c_0 = {}'.format(sol1))
print('m1 = {} '.format(sol0))
print('m2 = {}'.format(sol2))
print('m3 = {}'.format(sol3))
print('m4 = {}'.format(sol4))
print('t_1 = {}'.format(sol5))
print('t_2 = {}'.format(sol6))
print('t_3 = {}'.format(sol7))
# =============================================================================
...
norm_residuals = (rho_q_pres_array - fit_line_rho)/rho_fit_errors
