...
starting_directory = r'C:\Users\chipe\Documents\Durham University\warm_pixels\scripts\2024_july_opt2_beta584_notch511_rhoq'
...
#plt.savefig(f'{init_lag}/Beta(MJD)', bbox_inches="tight")
#plt.savefig(f'{init_lag}/Beta(MJD).pdf', bbox_inches="tight")
plt.show()
...
ax.tick_params(axis='both', which='major', labelsize=12)
ax_day.tick_params(axis='both', which='major', labelsize=12)
#plt.savefig(f'{init_lag}/BICS', bbox_inches="tight")
#plt.savefig(f'{init_lag}/BICS.pdf', bbox_inches="tight")
plt.show()
...
ax.tick_params(axis='both', which='major', labelsize=12)
ax_day.tick_params(axis='both', which='major', labelsize=12)
#plt.savefig(f'{init_lag}/a,b,c(MJD)', bbox_inches="tight")
#plt.savefig(f'{init_lag}/a,b,c(MJD).pdf', bbox_inches="tight")
plt.show()
...
ax.tick_params(axis='both', which='major', labelsize=12)
ax_day.tick_params(axis='both', which='major', labelsize=12)
#plt.savefig(f'{init_lag}/tau_a,tau_b,tau_c(MJD)', bbox_inches="tight")
#plt.savefig(f'{init_lag}/tau_a,tau_b,tau_c(MJD).pdf', bbox_inches="tight")
plt.show()
...
ax.tick_params(axis='both', which='major', labelsize=12)
ax_day.tick_params(axis='both', which='major', labelsize=12)
#plt.savefig(f'{init_lag}/CCDGAIN(MJD)', bbox_inches="tight")
#plt.savefig(f'{init_lag}/CCDGAIN(MJD).pdf', bbox_inches="tight")
plt.show()
...
#plt.savefig(f'{init_lag}/correction_metrics(MJD)', bbox_inches="tight")
#plt.savefig(f'{init_lag}/correction_metrics(MJD).pdf', bbox_inches="tight")
plt.show()
...
ax.tick_params(axis='both', which='major', labelsize=12)
#plt.savefig(f'{init_lag}/Notch(MJD)', bbox_inches="tight")
#plt.savefig(f'{init_lag}/Notch(MJD).pdf', bbox_inches="tight")
plt.show()
...
best_fit_lags=[]
best_fit_lags_err=[]
#test_inits = [0,50,100,150,200,250,300,350,400,450,500,550,600,650,700,750,800,850,900,950,1000]
test_inits = [0]
for init in test_inits:
    print("BEGIN SUNSPOT ANALYSIS")
    print(f"Initial guess for the lag in days is {init}")
    # Find sunspot data 
    sun_years=[]
    sun_months=[]
    sun_days=[]
    ...
    csv_file = r'C:\Users\chipe\Documents\Durham University\warm_pixels\scripts\sunspot_daily_2025.csv'
    ...
    sunspot_fit_array = np.array(sunspots, dtype=float)
    sunspot_days_since_launch_fit_array = np.array(sunspot_days_since_launch,dtype=float)
    
    sunspot_interp = interp1d(sunspot_days_since_launch_fit_array, sunspot_fit_array, 
                              kind='linear', fill_value='extrapolate')
    interp = PchipInterpolator(sunspot_days_since_launch_fit_array, sunspot_fit_array, extrapolate=True)
    interp_derivative = interp.derivative()
    
    
    print("Sunspot data extracted from csv file and interpolated!")
            
            
    def sunspot_rho_q(dates_since_launch, param_vals):    
        rho_q_vals = []
        #print("param_vals is: ", param_vals)
        for date in dates_since_launch:
            # Generate times between ACS launch minus lag and current date minus lag
            mask = (
            (sunspot_days_since_launch_fit_array + int(param_vals[4]) >= 0) &
            (sunspot_days_since_launch_fit_array <= date - int(param_vals[4]))
        )
            day_indices = np.where(mask)[0]  # This gives you the integer indices
            
            #print(day_indices)
            
            selected_spots_1 = sunspot_fit_array[day_indices]
            rho_q_1 = np.sum(param_vals[0] + param_vals[1] * np.exp(-param_vals[2] * (selected_spots_1 - param_vals[3])))
            
            # Now duplicate rho_q_1 but with days + 1
            selected_spots_2 = sunspot_fit_array[day_indices+1]
            rho_q_2 = np.sum(param_vals[0] + param_vals[1] * np.exp(-param_vals[2] * (selected_spots_2 - param_vals[3])))
            
            # Interpolate between rho_q_1 and rho_q_2
            t_frac = param_vals[4] - int(param_vals[4]) # Extract the fractional part
            
            rho_q_interp = (t_frac*rho_q_1) + ((1-t_frac)*rho_q_2)
            
            rho_q_vals.append(rho_q_interp)
    
        return np.array(rho_q_vals)
    
    def sunspot_rho_q_simple(dates_since_launch, param_vals):    
            
        # Assume a fit of the form y=mx+b+some offset from a fraction of the daily sunspots
        rho_q_calc = param_vals[0]*np.array(dates_since_launch) + param_vals[1] + sunspot_interp(dates_since_launch-param_vals[4])*param_vals[2]+param_vals[3]
    
        return np.array(rho_q_calc)
    
    def sunspot_rho_q_derivative(dates_since_launch, param_vals):    
            
        # Assume a fit of the form y=mx+b+some offset from a fraction of the daily sunspots derivative
        rho_q_calc = param_vals[0]*np.array(dates_since_launch) + param_vals[1] + interp_derivative(dates_since_launch-param_vals[4])*param_vals[2]+param_vals[3]
    ...
    print('ORIGINAL SUNSPOT RHO_Q FIT RESULTS')
    initial_values=np.array([0,0,0,0,float(init)])
    #initial_values=np.array([0,0,0,0,350])
...
# =============================================================================
    ...
    print("Number of iterations:", fit_sunspot.nit)
    print("Number of function evaluations:", fit_sunspot.nfev)
    sol0 = fit_sunspot.x[0]
    sol1 = fit_sunspot.x[1]
    sol2 = fit_sunspot.x[2]
    sol3 = fit_sunspot.x[3]
    lag_term = fit_sunspot.x[4]
    fit_sunspot_line = sunspot_rho_q(days, [sol0,sol1,sol2,sol3,lag_term])
    ...
    lag_err=errs_Hessian[4]
    
    
    print('minimised chi-squared = {}'.format(fit_sunspot.fun))
    chisq_min = fit_sunspot.fun
    chisq_reduced = chisq_min/deg_freedom
    reduced_chi_squared.append(chisq_reduced)
    best_fit_lags.append(lag_term)
    best_fit_lags_err.append(lag_err)
    print('reduced chi^2 = {}'.format(chisq_reduced))
    P_value = scipy.stats.chi2.sf(chisq_min, deg_freedom)
    print('P(chi^2_min, DoF) = {}'.format(P_value))
    print('First coefficient = {} +/- {}'.format(sol0, zero_err))
    ...
    print('Lag Term = {} +/- {}'.format(lag_term, lag_err))
    
    print('SIMPLE SUNSPOT RHO_Q FIT RESULTS')
    initial_values_simple=np.array([0,0,0,0,float(init)])
    deg_freedom_simple = len(days) - initial_values_simple.size
    print('DoF = {}'.format(deg_freedom_simple))
    fit_sunspot_simple = scipy.optimize.minimize(chi_squared, initial_values_simple, args=(sunspot_rho_q_simple, days, rho_q_pres_array, 
                                                                      sunspot_error_array))
    print(fit_sunspot_simple.success) 
    print(fit_sunspot_simple.message) 
    print("Number of iterations:", fit_sunspot_simple.nit)
    print("Number of function evaluations:", fit_sunspot_simple.nfev)
    sol0_simple = fit_sunspot_simple.x[0]
    sol1_simple = fit_sunspot_simple.x[1]
    sol2_simple = fit_sunspot_simple.x[2]
    sol3_simple = fit_sunspot_simple.x[3]
    sol4_simple = fit_sunspot_simple.x[4]
    fit_sunspot_line_simple = sunspot_rho_q_simple(days, [sol0_simple,sol1_simple,sol2_simple,sol3_simple, sol4_simple])
    
    #Show fit results
    errs_Hessian_simple = np.sqrt(np.diag(2*fit_sunspot_simple.hess_inv))
    
    zero_err_simple = errs_Hessian_simple[0]
    one_err_simple=errs_Hessian_simple[1]
    two_err_simple=errs_Hessian_simple[2]
    three_err_simple=errs_Hessian_simple[3]
    four_err_simple=errs_Hessian_simple[4]
    
    
    print('minimised chi-squared = {}'.format(fit_sunspot_simple.fun))
    chisq_min_simple = fit_sunspot_simple.fun
    chisq_reduced_simple = chisq_min_simple/deg_freedom_simple
# =============================================================================
#     reduced_chi_squared.append(chisq_reduced)
#     best_fit_lags.append(lag_term)
#     best_fit_lags_err.append(lag_err)
# =============================================================================
    print('reduced chi^2 = {}'.format(chisq_reduced_simple))
    P_value_simple = scipy.stats.chi2.sf(chisq_min_simple, deg_freedom_simple)
    print('P(chi^2_min, DoF) = {}'.format(P_value_simple))
    print('First coefficient = {} +/- {}'.format(sol0_simple, zero_err_simple))
    print('Second coefficient = {} +/- {}'.format(sol1_simple, one_err_simple))
    print('Third coefficient = {} +/- {}'.format(sol2_simple, two_err_simple))
    print('Fourth coefficient = {} +/- {}'.format(sol3_simple, three_err_simple))
    print('Lag in Days = {} +/- {}'.format(sol4_simple, four_err_simple))
    
    print('DERIVATIVE SUNSPOT RHO_Q FIT RESULTS')
    initial_values_derivative=np.array([0,0,0,0,float(init)])
    deg_freedom_derivative = len(days) - initial_values_derivative.size
    print('DoF = {}'.format(deg_freedom_derivative))
    fit_sunspot_derivative = scipy.optimize.minimize(chi_squared, initial_values_derivative, args=(sunspot_rho_q_derivative, days, rho_q_pres_array, 
                                                                      sunspot_error_array))
    print(fit_sunspot_derivative.success) 
    print(fit_sunspot_derivative.message) 
    print("Number of iterations:", fit_sunspot_derivative.nit)
    print("Number of function evaluations:", fit_sunspot_derivative.nfev)
    sol0_derivative = fit_sunspot_derivative.x[0]
    sol1_derivative = fit_sunspot_derivative.x[1]
    sol2_derivative = fit_sunspot_derivative.x[2]
    sol3_derivative = fit_sunspot_derivative.x[3]
    sol4_derivative = fit_sunspot_derivative.x[4]
    fit_sunspot_line_derivative = sunspot_rho_q_derivative(days, [sol0_derivative,sol1_derivative,sol2_derivative,sol3_derivative, sol4_derivative])
    
    #Show fit results
    errs_Hessian_derivative = np.sqrt(np.diag(2*fit_sunspot_derivative.hess_inv))
    
    zero_err_derivative = errs_Hessian_derivative[0]
    one_err_derivative=errs_Hessian_derivative[1]
    two_err_derivative=errs_Hessian_derivative[2]
    three_err_derivative=errs_Hessian_derivative[3]
    four_err_derivative=errs_Hessian_derivative[4]
    
    
    print('minimised chi-squared = {}'.format(fit_sunspot_derivative.fun))
    chisq_min_derivative = fit_sunspot_derivative.fun
    chisq_reduced_derivative = chisq_min_derivative/deg_freedom_derivative
# =============================================================================
#     reduced_chi_squared.append(chisq_reduced)
#     best_fit_lags.append(lag_term)
#     best_fit_lags_err.append(lag_err)
# =============================================================================
    print('reduced chi^2 = {}'.format(chisq_reduced_derivative))
    P_value_derivative = scipy.stats.chi2.sf(chisq_min_derivative, deg_freedom_derivative)
    print('P(chi^2_min, DoF) = {}'.format(P_value_derivative))
    print('First coefficient = {} +/- {}'.format(sol0_derivative, zero_err_derivative))
    print('Second coefficient = {} +/- {}'.format(sol1_derivative, one_err_derivative))
    print('Third coefficient = {} +/- {}'.format(sol2_derivative, two_err_derivative))
    print('Fourth coefficient = {} +/- {}'.format(sol3_derivative, three_err_derivative))
    print('Lag in Days = {} +/- {}'.format(sol4_derivative, four_err_derivative))
    
    print("END SUNSPOT ANALYSIS")
    ...
    ax.scatter(days, fit_sunspot_line_simple, color='chocolate', zorder=12)
    ax.scatter(days, fit_sunspot_line_derivative, color='crimson', zorder=12)
    ax.set_xlabel("Days since launch", fontsize=12)
    ax.plot(days_array, fit_line, linestyle='solid', color='orange')
    ax.set_xlim(-500, max(days)+500)
    ...
    plt.savefig(f'test_lags/Rho_q(MJD)_{init}', bbox_inches="tight")
    plt.savefig(f'test_lags/Rho_q(MJD)_{init}.pdf', bbox_inches="tight")
    plt.show()
    ...
    plt.ylim(-25,25)
    plt.gca().axes.get_yaxis().set_ticks([])
    plt.savefig(f'test_lags/Rho_q_post_zoom(MJD)_{init}', bbox_inches="tight")
    plt.savefig(f'test_lags/Rho_q_post_zoom(MJD)_{init}.pdf', bbox_inches="tight")
    plt.show()
...
test_inits = [0,50,100,150,200,250,300,350,400,450,500,550,600,650,700,750,800,850,900,950]

fig, ax = plt.subplots(figsize=(15, 8))
ax.scatter(test_inits, reduced_chi_squared ,color='green')
#ax.errorbar(hours, pressure, y_err=0.3*pressure, fmt='o')
#ax.vlines(5, ymin=0, ymax=1e-3)
#ax.vlines(38, ymin=0, ymax=1e-3)
ax.set_xlabel('Initial guess for lag (days)', fontsize=17)
#ax.set_xlim(93,100)
ax.set_ylabel('Reduced Chi Sq.', fontsize=17)
ax.tick_params(axis='both', which='major', labelsize=17)
plt.title("Initial lag guess vs reduced chi squared", fontsize=17)
#plt.savefig('CAM_best_leaktest_log.pdf')
#plt.savefig('CAM_best_leaktest_log.png')
plt.show()

fig, ax = plt.subplots(figsize=(15, 8))
ax.errorbar(test_inits, best_fit_lags, yerr=best_fit_lags_err ,color='red',fmt='o')
#ax.errorbar(hours, pressure, y_err=0.3*pressure, fmt='o')
#ax.vlines(5, ymin=0, ymax=1e-3)
#ax.vlines(38, ymin=0, ymax=1e-3)
ax.set_xlabel('Initial guess for lag (days)', fontsize=17)
#ax.set_xlim(93,100)
ax.set_ylabel('Best fit lag (days)', fontsize=17)
ax.tick_params(axis='both', which='major', labelsize=17)
plt.title("Initial lag guess vs best fit lag", fontsize=17)
#plt.savefig('CAM_best_leaktest_log.pdf')
#plt.savefig('CAM_best_leaktest_log.png')
plt.show()

fig, ax = plt.subplots(figsize=(15, 8))
ax.scatter(best_fit_lags, reduced_chi_squared ,color='blue')
#ax.errorbar(hours, pressure, y_err=0.3*pressure, fmt='o')
#ax.vlines(5, ymin=0, ymax=1e-3)
#ax.vlines(38, ymin=0, ymax=1e-3)
ax.set_xlabel('Best fit lag (days)', fontsize=17)
#ax.set_xlim(93,100)
ax.set_ylabel('Reduced Chi Sq.', fontsize=17)
ax.tick_params(axis='both', which='major', labelsize=17)
plt.title("Best fit lag vs reduced chi squared", fontsize=17)
#plt.savefig('CAM_best_leaktest_log.pdf')
#plt.savefig('CAM_best_leaktest_log.png')
plt.show()
...
# Save to CSV
df.to_csv('2024_july_opt2_beta584_notch511_rhoq_OUTPUTS.csv', index=False)