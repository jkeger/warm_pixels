...
# =============================================================================
    ...
    if days_var > temp_switch_date_since_launch:
        rho_q=0.0003711059187441403*days_var+0.07621630678335789
        a=1.4738781637492393e-05*days_var+0.05362877072713812
        b=7.647916477970915e-05*days_var+0.12269476639670227
        C=1-(a+b)
        tau_a = 1.0972005444568735e-05*days_var+0.4512593550211623
        tau_b = 0.00015586227317666347*days_var+4.382290325095798
        tau_c = -0.0026296120444556205*days_var+60.28560095720616
    elif days_var < temp_switch_date_since_launch:
        rho_q=0.00036862386204472716*days_var+0.014660115838463172
        a=8.171593239170827e-06*days_var+0.19205246768439355
        b=0.00020711738247067242*days_var+0.2668975750131912
        C=1-(a+b)
        tau_a=8.038769512346201e-05*days_var+0.48896841313905265
        tau_b=0.0006757683713399689*days_var+3.539481835795402
        tau_c=-0.005392385671986166*days_var+37.85160349469085
...
# =============================================================================
    ...
    writefilename=f"{dataset_date}_notch_pushed_opt3_{const_fix}" 
    ...
    csv_list=[x for x in csvs_string if f"{dataset_date}_notch_pushed_opt3_{const_fix}" in x]
    print(csv_list)
    csv_name=str(os.path.basename(csv_list[0]))
    print(csv_name)
    target2=path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", f"notch_pushed_opt3_{const_fix}", "csv_files",
                     str(csv_name))
...
cosma_output_path = path.join(cosma_path, "paolo",f"notch_pushed_opt3_{const_fix}")
...
dir = os.path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", f"notch_pushed_opt3_{const_fix}")
if not os.path.exists(dir):
    os.mkdir(dir)

dir = os.path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", f"notch_pushed_opt3_{const_fix}",
                 f"{dataset_date}_notch_pushed_opt3_{const_fix}")
if not os.path.exists(dir):
    os.mkdir(dir)
...
Paolo_autofit_global_50(
    group,
    save_path=Path(path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", f"notch_pushed_opt3_{const_fix}",
                     f"{dataset_date}_notch_pushed_opt3_{const_fix}"))/f"{dataset_date}_notch_pushed_opt3_{const_fix}.png"
)
...
for file in files_bia:
    ...
    output_path = path.join(path.sep, "cosma5", "data", "durham", "rjm","paolo", f"notch_pushed_opt3_{const_fix}", 
                            f"{dataset_date}_notch_pushed_opt3_{const_fix}", filename)
...
for file in files:
    ...
    image_A, image_B, image_C, image_D = [
        ImageACS.from_fits(
            ...
            bias_file_path=path.join(path.sep, "cosma5", "data", "durham", "rjm","paolo", f"notch_pushed_opt3_{const_fix}", 
                                    f"{dataset_date}_notch_pushed_opt3_{const_fix}")
        ).native
        for quadrant in ["A", "B", "C", "D"]
    ]
...
# =============================================================================
    ...
    #  Print results to csv file 
    writefilename=f"{dataset_date}_notch_pushed_opt3_{const_fix}_corrected"
    with open(writefilename+'.csv', 'w', newline='') as file:
        writer = csv.writer(file)
    ...
    for stuff in csvs_all:
        csvs_string.append(str(stuff))
    csv_list=[x for x in csvs_string if f"{dataset_date}_notch_pushed_opt3_{const_fix}_corrected" in x]
    print(csv_list)
    csv_name=str(os.path.basename(csv_list[0]))
    print(csv_name)
    target3=path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo",f"notch_pushed_opt3_{const_fix}",
                     "csv_files", str(csv_name))
    shutil.copyfile(csv_list[0],target3)

# Import data to be fitted
cosma_dataset_path = path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo",f"notch_pushed_opt3_{const_fix}",
                               f"{dataset_date}_notch_pushed_opt3_{const_fix}")
...
Paolo_autofit_global_50_after(
    group,
    save_path=Path(path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", f"notch_pushed_opt3_{const_fix}",
                     f"{dataset_date}_notch_pushed_opt3_{const_fix}"))/f"{dataset_date}_notch_pushed_opt3_{const_fix}_corrected.png"
)
