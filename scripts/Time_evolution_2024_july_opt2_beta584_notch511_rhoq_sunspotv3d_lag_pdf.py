...
def sunspot_rho_q(dates_since_launch, param_vals):    
    ...
    for date in dates_since_launch:
        ...
        rho_q = np.sum(param_vals[0] + param_vals[1] * np.exp(-param_vals[2] * (selected_spots - param_vals[3])))
...
initial_values=np.array([0,0,0,0,float(init_lag)])
...
plt.savefig(f'{init_lag}/Rho_q(MJD)', bbox_inches="tight")
plt.savefig(f'{init_lag}/Rho_q(MJD).pdf', bbox_inches="tight")
plt.show()
...
plt.ylim(-25,25)
plt.gca().axes.get_yaxis().set_ticks([])
plt.savefig(f'{init_lag}/Rho_q_post_zoom(MJD)', bbox_inches="tight")
plt.savefig(f'{init_lag}/Rho_q_post_zoom(MJD).pdf', bbox_inches="tight")
plt.show()
