...
class HeaderACS(Header):
    ...
    def paolo_array_original_to_electrons(self, array, use_calibrated_gain):
        ...
        if self.original_units in "COUNTS":
            array = (array * paolo_bscale) + paolo_bzero
...
# =============================================================================
    ...
    writefilename=f"{dataset_date}_stock_bia_v2" 
    ...
    batch_path = path.join(path.sep, "cosma5", "data", "durham", "rjm","paolo","dc-barr6", "warm_pixels_workspace", 
                           "batch_scripts", "stock_bia_v2")
    ...
    csv_list=[x for x in csvs_string if f"{dataset_date}_stock_bia_v2" in x]
    print(csv_list)
    csv_name=str(os.path.basename(csv_list[0]))
    print(csv_name)
    target2=path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", "stock_bia_v2", "csv_files",
                     str(csv_name))
...
cosma_output_path = path.join(cosma_path, "paolo","stock_bia_v2")
...
dir = os.path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", "stock_bia_v2",
                 f"{dataset_date}_stock_bia_v2")
...
Paolo_autofit_global_50(
    group,
    save_path=Path(path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", "stock_bia_v2",
                     f"{dataset_date}_stock_bia_v2"))/f"{dataset_date}_stock_bia_v2.png"
)
...
# Find all the fits files. Correct science images first. 
...
for file in files:
    ...
    output_path = path.join(path.sep, "cosma5", "data", "durham", "rjm","paolo", "stock_bia_v2", 
                            f"{dataset_date}_stock_bia_v2", filename)
...
# =============================================================================
    ...
    #  Print results to csv file 
    writefilename=f"{dataset_date}_stock_bia_v2_corrected"
    with open(writefilename+'.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([f"Log likelihood = {result.log_likelihood}"])
        writer.writerow([f"beta = {best_trail_model.beta}"])
    ...
    batch_path = path.join(path.sep, "cosma5", "data", "durham", "rjm","paolo","dc-barr6",
                           "warm_pixels_workspace", "batch_scripts", "stock_bia_v2")
    csv_directory = Path(batch_path)
    csvs_all=list(pathlib.Path(csv_directory).glob('*.csv'))
    csvs_string=[]
    for stuff in csvs_all:
        csvs_string.append(str(stuff))
    csv_list=[x for x in csvs_string if f"{dataset_date}_stock_bia_v2_corrected" in x]
    print(csv_list)
    csv_name=str(os.path.basename(csv_list[0]))
    print(csv_name)
    target3=path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo","stock_bia_v2",
                     "csv_files", str(csv_name))
    shutil.copyfile(csv_list[0],target3)

# Import data to be fitted
cosma_dataset_path = path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo","stock_bia_v2",
                               f"{dataset_date}_stock_bia_v2")
...
Paolo_autofit_global_50_after(
    group,
    save_path=Path(path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", "stock_bia_v2",
                     f"{dataset_date}_stock_bia_v2"))/f"{dataset_date}_stock_bia_v2_corrected.png"
)
