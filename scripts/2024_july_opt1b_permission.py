...
# =============================================================================
    ...
    best_fit_capt_b=best_trail_model.capt_a
    best_fit_capt_c=best_trail_model.capt_a
...
# =============================================================================
                ...
                global_autofit=trail_model_arctic_notch_pushed_plot_slowcap(x=pixels, 
                                           ...
                                           capt_b=best_fit_capt_a,
                                           capt_c=best_fit_capt_a,
                                           notch=best_fit_notch
                                          )
                print('Done!')
    ...
    writefilename=f"{dataset_date}_2024_july_opt1b_{const_fix}" 
    ...
    csv_list=[x for x in csvs_string if f"{dataset_date}_2024_july_opt1b_{const_fix}" in x]
    print(csv_list)
    csv_name=str(os.path.basename(csv_list[0]))
    print(csv_name)
    target2=path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", f"2024_july_opt1b_{const_fix}", "csv_files",
                     str(csv_name))
...
cosma_output_path = path.join(cosma_path, "paolo",f"2024_july_opt1b_{const_fix}")
...
dir = os.path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", f"2024_july_opt1b_{const_fix}")
if not os.path.exists(dir):
    os.mkdir(dir)

dir = os.path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", f"2024_july_opt1b_{const_fix}",
                 f"{dataset_date}_2024_july_opt1b_{const_fix}")
if not os.path.exists(dir):
    os.mkdir(dir)
...
Paolo_autofit_global_50(
    group,
    save_path=Path(path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", f"2024_july_opt1b_{const_fix}",
                     f"{dataset_date}_2024_july_opt1b_{const_fix}"))/f"{dataset_date}_2024_july_opt1b_{const_fix}.png"
)
...
for file in files_bia:
    ...
    output_path = path.join(path.sep, "cosma5", "data", "durham", "rjm","paolo", f"2024_july_opt1b_{const_fix}", 
                            f"{dataset_date}_2024_july_opt1b_{const_fix}", filename)
...
for file in files:
    ...
    image_A, image_B, image_C, image_D = [
        ImageACS.from_fits(
            ...
            bias_file_path=path.join(path.sep, "cosma5", "data", "durham", "rjm","paolo", f"2024_july_opt1b_{const_fix}", 
                                    f"{dataset_date}_2024_july_opt1b_{const_fix}")
        ).native
        for quadrant in ["A", "B", "C", "D"]
    ]
...
# =============================================================================
    ...
    #  Print results to csv file 
    writefilename=f"{dataset_date}_2024_july_opt1b_{const_fix}_corrected"
    with open(writefilename+'.csv', 'w', newline='') as file:
        writer = csv.writer(file)
    ...
    for stuff in csvs_all:
        csvs_string.append(str(stuff))
    csv_list=[x for x in csvs_string if f"{dataset_date}_2024_july_opt1b_{const_fix}_corrected" in x]
    print(csv_list)
    csv_name=str(os.path.basename(csv_list[0]))
    print(csv_name)
    target3=path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo",f"2024_july_opt1b_{const_fix}",
                     "csv_files", str(csv_name))
    shutil.copyfile(csv_list[0],target3)

# Import data to be fitted
cosma_dataset_path = path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo",f"2024_july_opt1b_{const_fix}",
                               f"{dataset_date}_2024_july_opt1b_{const_fix}")
...
Paolo_autofit_global_50_after(
    group,
    save_path=Path(path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", f"2024_july_opt1b_{const_fix}",
                     f"{dataset_date}_2024_july_opt1b_{const_fix}"))/f"{dataset_date}_2024_july_opt1b_{const_fix}_corrected.png"
)
