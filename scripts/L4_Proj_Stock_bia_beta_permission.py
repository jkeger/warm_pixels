...
# =============================================================================
    ...
    writefilename=f"{dataset_date}_stock_bia_beta" 
    ...
    csv_list=[x for x in csvs_string if f"{dataset_date}_stock_bia_beta" in x]
    print(csv_list)
    csv_name=str(os.path.basename(csv_list[0]))
    print(csv_name)
    target2=path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", "stock_bia_beta", "csv_files",
                     str(csv_name))
...
cosma_output_path = path.join(cosma_path, "paolo","stock_bia_beta")
...
dir = os.path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", "stock_bia_beta")
if not os.path.exists(dir):
    os.mkdir(dir)

dir = os.path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", "stock_bia_beta",
                 f"{dataset_date}_stock_bia_beta")
if not os.path.exists(dir):
    os.mkdir(dir)
...
Paolo_autofit_global_50(
    group,
    save_path=Path(path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", "stock_bia_beta",
                     f"{dataset_date}_stock_bia_beta"))/f"{dataset_date}_stock_bia_beta.png"
)
...
for file in files_bia:
    ...
    output_path = path.join(path.sep, "cosma5", "data", "durham", "rjm","paolo", "stock_bia_beta", 
                            f"{dataset_date}_stock_bia_beta", filename)
...
for file in files:
    ...
    image_A, image_B, image_C, image_D = [
        ImageACS.from_fits(
            ...
            bias_file_path=path.join(path.sep, "cosma5", "data", "durham", "rjm","paolo", "stock_bia_beta", 
                                    f"{dataset_date}_stock_bia_beta")
        ).native
        for quadrant in ["A", "B", "C", "D"]
    ]
...
# =============================================================================
    ...
    #  Print results to csv file 
    writefilename=f"{dataset_date}_stock_bia_beta_corrected"
    with open(writefilename+'.csv', 'w', newline='') as file:
        writer = csv.writer(file)
    ...
    for stuff in csvs_all:
        csvs_string.append(str(stuff))
    csv_list=[x for x in csvs_string if f"{dataset_date}_stock_bia_beta_corrected" in x]
    print(csv_list)
    csv_name=str(os.path.basename(csv_list[0]))
    print(csv_name)
    target3=path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo","stock_bia_beta",
                     "csv_files", str(csv_name))
    shutil.copyfile(csv_list[0],target3)

# Import data to be fitted
cosma_dataset_path = path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo","stock_bia_beta",
                               f"{dataset_date}_stock_bia_beta")
...
Paolo_autofit_global_50_after(
    group,
    save_path=Path(path.join(path.sep, "cosma5", "data", "durham", "rjm", "paolo", "stock_bia_beta",
                     f"{dataset_date}_stock_bia_beta"))/f"{dataset_date}_stock_bia_beta_corrected.png"
)
